...
class UserProductMvcTest {
    ...
    private Principal mockPrincipal; //가짜 인증해 줄 principal
    ...
    @BeforeEach
    public void setup() { //우리가 만들어준 가짜필터 넣어주기
    ...
    private void mockUserSetup() {
        ...
        User testUser = new User(username, password, email, role); //테스트할 유저 객체
        UserDetailsImpl testUserDetails = new UserDetailsImpl(testUser); //UserDetailsImpl 직접 만들어주기
        mockPrincipal = new UsernamePasswordAuthenticationToken(testUserDetails, "", testUserDetails.getAuthorities());
    }

    //Application에 @EnableAuditing 때문에 오류 생김
    @Test
    ...
    void test1() throws Exception {
        ...
        mvc.perform(get("/api/user/login-page"))
                ...
                .andExpect(view().name("login")) //반환되는 html파일
                .andDo(print());
    }
    ...
    void test2() throws Exception {
        ...
        mvc.perform(post("/api/user/signup")
                ...
                .andExpect(status().is3xxRedirection()) //3xx: 리다이렉트 예측
                .andExpect(view().name("redirect:/api/user/login-page")) //뷰 이름
                .andDo(print());
    }
    ...
    void test3() throws Exception {
        ...
        String postInfo = objectMapper.writeValueAsString(requestDto); //json -> String으로
